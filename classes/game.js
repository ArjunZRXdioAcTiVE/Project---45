class Game {
  ...
  play() {
    ...
    if (player.index == 1) {
      ...
      } else if (keyDown("space") && !this.ballCreated) {
        var cannonBall = new CannonBall();
        cannonBalls.push(cannonBall);
        console.log (cannonBalls);
        this.ballCreated = true;

        firebase.ref("players/player1/cannonBall").set ({
          created: true
        });
      } 
      
      if (keyDown ("s") && this.ballCreated && !this.shot) {
        cannonBalls[0].shoot(cannonLauncher1);
        this.shot = true;
      }

      if (this.ballCreated) {
        cannonBalls[0].displayBall();

        var ballPX = cannonBalls[0].ball.position
        firebase.ref("players/player1/cannonBall").update({
          posX: ballPX.x,
          posY: ballPX.y
        });
      }
